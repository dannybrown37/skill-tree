...
per unit of interface they have to learn. A module is **deep** when a large amount of behaviour
...
- **Depth as ratio of implementation-lines to interface-lines** (Ousterhout): rewards padding
  the implementation. We use depth-as-leverage instead.
...
- **Deepening a cluster given its dependencies**, see
  [references/DEEPENING.md](references/DEEPENING.md): dependency categories, seam discipline,
  and replace-don't-layer testing.
- **Exploring alternative interfaces**, see
  [references/DESIGN-IT-TWICE.md](references/DESIGN-IT-TWICE.md): spin up parallel agents to
