...
**Use less, pay less.** That is the entire root of Dynamo cost work. There is no pricing tier that saves you from a bad item shape.

Cost comes from six places. Find out which one dominates *before* optimizing anything: compute capacity, storage, backups, import/export, data transfer, streams.
...
- **Shorten attribute names.** Dynamo is schema-less, so every attribute name is stored on
  every item. The tradeoff is real — short names cost human mental mapping — so spend it on
...
- Data type matters a little — numbers store slightly smaller than strings — but it's noise
  next to item shape.
...
- **GSIs that haven't been read in 30 days are surprisingly common.** Audit read metrics per
  ...
  writes cost 5–20x reads — an unread index is a pure tax on every write.
- The decision rule:
...
- **On-demand** maximizes elasticity and minimizes throttling. On-demand throughput pricing was
  cut 50% in November 2024, which moved the break-even a long way in its favor — re-run any
...
- **Export to S3** for anything downstream. No read costs, no ETL pipeline to maintain — AWS
